...
app.post("/missionoutfeed", (req, res) => {
  ...
  const sql = "SELECT * FROM  missionoutfeed";
...
app.get("/exportExcel", (req, res) => {
  ...
  db.query(sql, (error, results) => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("MissionOutfeed");
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=MissionOutfeed.xlsx"
      );
...
app.get("/outfeed", (req, res) => {
  const statusCountsQuery = "SELECT Status,COUNT(Status) AS count FROM missionoutfeed GROUP BY Status";
