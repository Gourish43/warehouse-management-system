...
app.post("/partialinfeed", (req, res) => {
  ...
  const sql = "SELECT * FROM partialpalletinfeed";
...
app.get("/exportExcel", (req, res) => {
  const sql = "SELECT * FROM missioninfeed";
  ...
  db.query(sql, (error, results) => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("PartialPalletInfeed");
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=PartialPalletInfeed.xlsx"
      );
...
app.get("/statusCounts", (req, res) => {
  const statusCountsQuery ="SELECT Status,COUNT(Status) AS count FROM partialpalletinfeed GROUP BY Status";
