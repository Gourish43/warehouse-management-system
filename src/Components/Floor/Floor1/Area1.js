...
function CardComponent1() {
  ...
  const [cardLocks, setCardLocks] = useState(new Array(624).fill(false));
  const [cardClose, setCardClose] = useState(new Array(624).fill(false));
  const [cardList, setcardList] = useState([]);
  const [cardColors, setCardColors] = useState(new Array(624).fill('white'));
  ...
  const handleOpenDialog = (index, width) => {
    ...
    const selectedCard = `2AL${index + 1}`;
    ...
    fetch('http://localhost:5000/floor/addArea2', {
        ...
        body: JSON.stringify({
            ...
            area: 'area1', // Provide the area value (change this as needed)
        }),
    })
  ...
  const handleUnloadData = () => {
    if (!selectedCardIndex) {
        window.alert('Please select a rack before unloading data.');
        return;
    }
...
const handleAddProd = () => {
  ...
  if (cardList.length >= 2) {
      window.alert('Inventory is full. This Rack will be locked.');
...
const renderCardComponents = () => {
  ...
  const totalCards = Array.from({ length: 624 });
