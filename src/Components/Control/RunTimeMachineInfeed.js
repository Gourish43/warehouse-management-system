...
function RunTimeMachineInfeed() {
 ...
 useEffect(() => {
    axios
      .post("http://localhost:5000/machineinfeed/missioninfeed")
      .then((response) => {
      ...
      axios
      .get("http://localhost:5000/machineinfeed/statusCounts")
      .then((response) => {
...
const handleExportExcel = () => {
  axios
    .get("http://localhost:5000/machineinfeed/exportExcel", { responseType: "blob" })
    .then((response) => {
  ...
  const rows = data.map((missioninfeed) => ({
    id:missioninfeed.SlNo, // Assign the 'SlNo' as a unique identifier
    ...missioninfeed,
  }));
  return (
    <>
      <div className="show">
        <center>
          <h2 align="left">Runtime Mission Infeed</h2>
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;
      <Box sx={{ height: '40%', flex: '50%', marginTop: '10px' }}>
            <DataGrid
              rows={[
                {
                  id: 'statusCountsRow',
...
export default RunTimeMachineInfeed
