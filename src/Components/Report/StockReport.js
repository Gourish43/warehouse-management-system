...
function StockReport() {
 const [data, setData] = useState([]);
  ...
  useEffect(() => {
    axios
      .post("http://localhost:5000/stockreport/stockreport")
      .then((response) => {
  ...
  ? data.filter((stockreport) => stockreport.Product === selectedProduct)
  : data;

const rows = filteredRows.map((stockreport) => ({
  id: stockreport.SlNo,
  ...stockreport,
}));
...
return (
   ...
   {rows.length === 0 ? (
            <div>No data available {selectedProduct}</div>
          ) : (
            <DataGrid
              rows={rows}
              columns={columns}
