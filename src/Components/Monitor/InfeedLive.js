...
function InfeedLive() {
 ...
 useEffect(() => {
    axios
      .post("http://localhost:5000/infeedlive/liveinfeed")
      .then((response) => {
      ...
      axios
      .get("http://localhost:5000/infeedlive/statusCounts")
      .then((response) => {
...
const handleExportExcel = () => {
  axios
    .get("http://localhost:5000/infeedlive/exportExcel", { responseType: "blob" })
    .then((response) => {
  ...
  const rows = data.map((liveinfeed) => ({
    id:liveinfeed.SlNo, // Assign the 'SlNo' as a unique identifier
    ...liveinfeed,
  }));
  return (
    <>
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;
      <Box sx={{ height: '40%', width: '50%', marginTop: '10px' ,align:'center'}}>
            <DataGrid
              rows={[
                {
                  id: 'statusCountsRow',
